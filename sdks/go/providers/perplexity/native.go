...
const nativeBaseURL = "https://api.perplexity.ai"

// NativeClient is a Perplexity native provider client with CyberArmor hooks.
...
// NewNative builds a Perplexity native provider client.
...
// ChatCompletions executes Perplexity chat completions with policy enforcement.
func (c *NativeClient) ChatCompletions(
	...
	decision, err := c.CA.EvaluatePolicy(ctx, cyberarmor.EvaluatePolicyOptions{
		Action:     "provider.perplexity.chat_completions",
		Provider:   "perplexity",
		Model:      model,
		PromptText: prompt,
	})
	...
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("perplexity API error %d: %s", resp.StatusCode, string(data))
	}
	...
	c.CA.EmitEvent("provider.perplexity.chat_completions", map[string]any{
		"tenant_id":  tenantID,
		"model":      model,
