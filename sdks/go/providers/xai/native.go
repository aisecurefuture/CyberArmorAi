...
const nativeBaseURL = "https://api.x.ai/v1"

// NativeClient is an xAI native provider client with CyberArmor hooks.
...
// NewNative builds an xAI native provider client.
...
// ChatCompletions executes xAI chat completions with policy enforcement.
func (c *NativeClient) ChatCompletions(
	...
	decision, err := c.CA.EvaluatePolicy(ctx, cyberarmor.EvaluatePolicyOptions{
		Action:     "provider.xai.chat_completions",
		Provider:   "xai",
		Model:      model,
		PromptText: prompt,
	})
	...
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("xai API error %d: %s", resp.StatusCode, string(data))
	}
	...
	c.CA.EmitEvent("provider.xai.chat_completions", map[string]any{
		"tenant_id":  tenantID,
		"model":      model,
