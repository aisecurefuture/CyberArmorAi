...
_DLP_PATTERNS: List[Tuple[str, re.Pattern, str]] = [
    ...
    ("OPENAI_API_KEY",
     re.compile(r"\b(?:sk-(?:proj|svcacct)-[A-Za-z0-9_\-]{20,}|sk-[A-Za-z0-9]{32,})\b"),
     "[REDACTED-OPENAI-KEY]"),
    ("ANTHROPIC_API_KEY",
     re.compile(r"\bsk-ant-[A-Za-z0-9\-_]{60,}\b"),
    ...
    ("PASSWORD_FIELD",
     ...
     "[REDACTED-PASSWORD]"),
    ("API_KEY_GENERIC",
     re.compile(r"\b(?:[A-Za-z0-9]+_)*(?:api[_\-]?key|apikey|secret[_\-]?key|access[_\-]?token)\s*[:=]\s*['\"]?([A-Za-z0-9_\-]{16,})['\"]?",
                re.IGNORECASE),
     "[REDACTED-APIKEY]"),
    ("PRIVATE_KEY_BLOCK",
