...
const PUBLIC_KEY_CACHE = new Map<string, { info: PublicKeyInfo; expiresAt: number }>();
const HEADER_CACHE = new Map<string, { value: string; expiresAt: number }>();
...
function resolvePublicKeyUrl(baseUrl: string): string {
...
async function fetchPublicKeyInfo(baseUrl: string): Promise<PublicKeyInfo> {
  ...
  const info = (await response.json()) as PublicKeyInfo;
  PUBLIC_KEY_CACHE.set(url, { info, expiresAt: Date.now() + CACHE_TTL_MS });
  return info;
}

async function encryptMlKemHeader(apiKey: string, publicKeyHex: string): Promise<string> {
...
async function encryptX25519Header(apiKey: string, publicKeyHex: string): Promise<string> {
...
export async function buildAuthHeaderResult(baseUrl: string, apiKey: string, strict = false): Promise<AuthHeaderResult> {
  if (!apiKey) return { value: "", mode: "missing", algorithm: "none" };
  if (apiKey.startsWith("PQC:")) return { value: apiKey, mode: "precomputed_pqc", algorithm: "precomputed" };
  ...
  try {
    ...
    if (cached && cached.expiresAt > Date.now()) return { value: cached.value, mode: algorithmName.includes("ML-KEM-1024") ? "native_ml_kem" : "x25519_fallback", algorithm: algorithmName, cached: true };
...
export async function buildAuthHeaderValue(baseUrl: string, apiKey: string, strict = false): Promise<string> {
